...
// Stesso schema minimale di flags.json/bugs.json: un solo file JSON alla
// radice della repo dati, ma qui un oggetto piatto { id: label } invece di
// un array — overlay di termini iconografici non ancora curati a mano in
// iconographyLabels.ts (vedi mergeIconographyOverrides). Scritture attese
// rarissime (solo quando un editor introduce un termine nuovo), quindi
// stesso pattern "single retry" degli altri due file di supporto.
const ICONOGRAPHY_VOCAB_PATH = "iconography-vocab.json";
let iconographyVocabSha: string | null = null;

/** Legge iconography-vocab.json dalla repo. `null` se non esiste ancora. */
export async function pullIconographyVocabFileFromGitHub(): Promise<string | null> {
  if (!isGitHubConfigured()) return null;
  const cfg = getConfig();
  const url = `${GITHUB_API}/repos/${cfg.repo}/contents/${ICONOGRAPHY_VOCAB_PATH}?ref=${encodeURIComponent(cfg.branch)}`;
  const res = await fetch(url, { headers: headers(cfg) });
...
export async function pushIconographyVocabFileToGitHub(content: string, message: string): Promise<void> {
  if (!isGitHubConfigured()) return;
  const cfg = getConfig();
  const url = `${GITHUB_API}/repos/${cfg.repo}/contents/${ICONOGRAPHY_VOCAB_PATH}`;
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    const isRaceConflict = res.status === 409 || res.status === 422;
    if (isRaceConflict && attempt < 3) {
      console.warn(`[githubStorage] Conflitto (${res.status}) su iconography-vocab.json, tentativo ${attempt}/3, ritento...`);
      await new Promise(r => setTimeout(r, 150 * attempt));
      const refreshedRes = await fetch(`${url}?ref=${encodeURIComponent(cfg.branch)}`, { headers: headers(cfg), cache: "no-store" });
      sha = refreshedRes.ok ? (await refreshedRes.json()).sha : null;
      continue;
...
// Variante Node del file unico della sezione «Fonti letterarie»: vedi il
// commento gemello in githubStorageBrowser.ts per il perché di un solo file.
// Qui serve soprattutto in lettura, a scripts/build-corpus-snapshot.ts, che
// ne fa uno scatto statico per chi visita il sito senza sbloccare la
// redazione.
const LIT_SOURCES_PATH = "fonti-letterarie.json";
let litSourcesSha: string | null = null;

/** Legge fonti-letterarie.json dalla repo. `null` se non esiste ancora. */
export async function pullLitSourcesFileFromGitHub(): Promise<string | null> {
  if (!isGitHubConfigured()) return null;
  const cfg = getConfig();
  const url = `${GITHUB_API}/repos/${cfg.repo}/contents/${LIT_SOURCES_PATH}?ref=${encodeURIComponent(cfg.branch)}`;
  const res = await fetch(url, { headers: headers(cfg) });
  if (res.status === 404) { litSourcesSha = null; return null; }
  if (!res.ok) throw new Error(`GitHub get fonti-letterarie.json fallita (${res.status}): ${await res.text()}`);
  const data = await res.json();
  litSourcesSha = data.sha || null;
  if (data.encoding !== "base64" || typeof data.content !== "string") {
...
export async function pushLitSourcesFileToGitHub(content: string, message: string): Promise<void> {
  if (!isGitHubConfigured()) return;
  const cfg = getConfig();
  const url = `${GITHUB_API}/repos/${cfg.repo}/contents/${LIT_SOURCES_PATH}`;
  let sha = litSourcesSha;
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (res.ok) {
      const data = await res.json();
      litSourcesSha = data.content.sha;
      return;
    }
    ...
    const isRaceConflict = res.status === 409 || res.status === 422;
    if (isRaceConflict && attempt < 3) {
      console.warn(`[githubStorage] Conflitto (${res.status}) su fonti-letterarie.json, tentativo ${attempt}/3, ritento...`);
      await new Promise(r => setTimeout(r, 150 * attempt));
      const refreshedRes = await fetch(`${url}?ref=${encodeURIComponent(cfg.branch)}`, { headers: headers(cfg), cache: "no-store" });
      sha = refreshedRes.ok ? (await refreshedRes.json()).sha : null;
      continue;
...
let lessicoLaresSha: string | null = null;

export async function pullLessicoLaresFileFromGitHub(): Promise<string | null> {
  if (!isGitHubConfigured()) return null;
  const cfg = getConfig();
  const url = `${GITHUB_API}/repos/${cfg.repo}/contents/${LESSICO_LARES_PATH}?ref=${encodeURIComponent(cfg.branch)}`;
  const res = await fetch(url, { headers: headers(cfg) });
  if (res.status === 404) { lessicoLaresSha = null; return null; }
  if (!res.ok) throw new Error(`GitHub get lessico-lares-overlay.json fallita (${res.status}): ${await res.text()}`);
  const data = await res.json();
  lessicoLaresSha = data.sha || null;
  if (data.encoding !== "base64" || typeof data.content !== "string") {
...
export async function pushLessicoLaresFileToGitHub(content: string, message: string): Promise<void> {
  if (!isGitHubConfigured()) return;
  const cfg = getConfig();
  const url = `${GITHUB_API}/repos/${cfg.repo}/contents/${LESSICO_LARES_PATH}`;
  let sha = lessicoLaresSha;
  ...
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (res.ok) {
      const data = await res.json();
      lessicoLaresSha = data.content.sha;
      return;
    }
    ...
    const isRaceConflict = res.status === 409 || res.status === 422;
    if (isRaceConflict && attempt < 3) {
      console.warn(`[githubStorage] Conflitto (${res.status}) su lessico-lares-overlay.json, tentativo ${attempt}/3, ritento...`);
      await new Promise(r => setTimeout(r, 150 * attempt));
      const refreshedRes = await fetch(`${url}?ref=${encodeURIComponent(cfg.branch)}`, { headers: headers(cfg), cache: "no-store" });
      sha = refreshedRes.ok ? (await refreshedRes.json()).sha : null;
      continue;
    }
    throw new Error(`Scrittura GitHub fallita per lessico-lares-overlay.json (${res.status}) dopo ${attempt} tentativi: ${detail}`);
  }
}
