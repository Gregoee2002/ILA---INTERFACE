...
e registro onomastico, nella sottosezione 5.3 alla disposizione
cronologica delle attestazioni, e nella sottosezione 5.4 agli strumenti
di ricerca. Infine, nella sezione 6 si propongono alcune considerazioni
...
Distinta da questa tracciabilità puntuale dei singoli interventi, ma ad
essa complementare, è l'attribuzione di responsabilità intellettuale
sull'intera scheda: chi ne ha curato l'edizione, chi l'ha rivista, chi
si è occupato della codifica, e chi vi ha altrimenti contribuito. Anche
per questo tipo di informazione si è scelto di appoggiarsi a un elemento
già previsto dallo standard di codifica filologica anziché introdurre
un campo ad hoc, cosicché l'elenco dei responsabili di ciascuna voce,
con il rispettivo ruolo, resti espresso nella stessa forma normalizzata
in cui verrebbe letto da qualunque altro strumento conforme allo stesso
standard, e non in una convenzione locale al solo progetto ILA. Poiché i
ruoli editoriali ricorrenti sono in numero limitato, la loro
compilazione si appoggia a un suggerimento automatico costruito sui
valori già impiegati altrove nel corpus, secondo lo stesso principio,
già discusso a proposito del vocabolario iconografico (sottosezione
5.1), per cui lasciare che un insieme ristretto di valori coerenti
emerga dall'uso concreto è preferibile a un campo di testo libero, che
finirebbe inevitabilmente per accumulare varianti equivalenti ma
formalmente distinte di uno stesso ruolo.
...
distribuzione spaziale delle attestazioni su una mappa interattiva; una
sezione cronologica, descritta più diffusamente nella sottosezione 5.3,
che dispone le medesime attestazioni lungo l'asse del tempo anziché
dello spazio; e una sezione di ricerca avanzata, descritta più
diffusamente nella sottosezione 5.4. Ogni scheda descrittiva è
corredata, ove disponibile,
...
\subsection{La cronologia del corpus}

Accanto alla distribuzione spaziale delle attestazioni, resa esplicita
dalla sezione geografica, e alla loro organizzazione onomastica appena
descritta, resta una terza dimensione lungo cui un corpus di questo
tipo chiede di essere osservato: quella temporale, ossia il modo in cui
le epigrafi si distribuiscono nei secoli attraversati dal culto di Men.
A questo scopo l'interfaccia dispone di una vista cronologica dedicata,
che riprende deliberatamente il linguaggio visivo dei diagrammi di
pianificazione a barre: ogni epigrafe datata è rappresentata da una
barra ancorata alla propria collocazione temporale reale, mai spostata
per far posto ad altre voci, e lunga quanto l'intervallo di datazione
proposto per quella scheda --- cosicché un'attestazione databile con
precisione a un singolo anno appare visibilmente più stretta di una
databile solo entro un arco di più decenni, restituendo a colpo d'occhio
il grado di incertezza cronologica insieme alla collocazione nel tempo.
Quando due o più epigrafi si sovrappongono nei rispettivi intervalli di
datazione, le barre corrispondenti non vengono fuse né nascoste l'una
dietro l'altra, ma impilate ciascuna nella prima fascia libera al di
sotto delle precedenti, in modo che ogni singola attestazione resti
individualmente visibile e selezionabile anche nei periodi di maggiore
densità documentaria.

Poiché l'intera sequenza cronologica del corpus, letta per intero alla
massima risoluzione, non potrebbe mai essere contenuta in una sola
schermata, la vista adotta un unico continuo di ingrandimento che
attraversa senza soluzione di continuità due livelli di lettura. Al
livello minimo, quello proposto di default all'apertura della sezione,
i secoli sono resi come blocchi compatti accostati l'uno all'altro, ciascuno
corredato del solo numero di attestazioni che vi ricadono: l'intero arco
cronologico del corpus resta così visibile per intero, senza necessità
di scorrimento, offrendo un primo colpo d'occhio sulla distribuzione
diacronica del culto. Superata una soglia di ingrandimento, la vista
transita gradualmente al livello di dettaglio, in cui riappaiono le
singole barre impilate descritte sopra, ciascuna corredata
dell'identificativo della scheda corrispondente. Un clic diretto su un
blocco-secolo porta immediatamente al livello di dettaglio centrato su
quel secolo, cosicché la lettura d'insieme e l'approfondimento puntuale
restano due momenti dello stesso gesto di navigazione, anziché due
strumenti separati tra cui scegliere in anticipo. L'ingrandimento può
inoltre essere regolato in modo continuo, con la rotellina del mouse o
il gesto di pizzico sul touchpad tenendo premuto un tasto modificatore,
mantenendo fermo sotto il cursore l'anno su cui si sta agendo; un
comando dedicato riporta in ogni momento alla vista d'insieme iniziale,
che costituisce anche il limite minimo di rimpicciolimento raggiungibile,
non avendo senso allontanarsi oltre il punto in cui l'intera sequenza è
già interamente visibile.

\screenshotplaceholder{7cm}{Vista cronologica del corpus, a livello di panoramica per secolo e a livello di dettaglio}
...
sottosezione 5.1, singoli descrittori iconografici. È inoltre
...
solo in forma discorsiva nella letteratura di riferimento --- due viste
...
e una vista cronologica che restituisce la medesima distribuzione del
corpus lungo l'asse del tempo anziché lungo quello, già coperto dalla
sezione geografica, dello spazio.
...
principalmente su tre fronti. Il primo è la revisione sistematica, voce
per voce, delle trascrizioni derivate dall'edizione a stampa originaria,
sulla base di un confronto diretto con le fonti primarie disponibili,
così da ridurre progressivamente la dipendenza dal testo di Lane e
trasformarlo, per ciascuna voce revisionata, da testo sottostante a
semplice riferimento bibliografico. Il secondo è l'ampliamento del
vocabolario controllato utilizzato per la descrizione iconografica, e lo
sviluppo di strumenti di confronto comparativo tra le diverse
attestazioni iconografiche del dio Men, in modo da valorizzare appieno
il potenziale interrogativo di questa sezione dell'interfaccia digitale.

Il terzo fronte, di respiro più ampio, riguarda il testo stesso delle
epigrafi. La ricerca testuale descritta nella sottosezione 5.4 opera
attualmente a livello di forma attestata --- normalizzata nei segni
diacritici, ma pur sempre la sequenza di caratteri effettivamente
incisa sul monumento --- e non è quindi in grado, allo stato attuale, di
riconoscere come equivalenti forme flesse diverse di uno stesso vocabolo:
chi cerchi tutte le occorrenze di un determinato termine deve conoscerne
in anticipo, e interrogare separatamente, le singole forme in cui esso
compare nel corpus. Si prevede pertanto di intraprendere la
lemmatizzazione completa del testo epigrafico, ossia il collegamento
sistematico di ciascuna forma attestata al proprio lemma di riferimento,
sul modello di quanto già avviene, a un diverso livello, per
l'iconografia e per l'onomastica del corpus: così come una chiave
iconografica controllata rende confrontabili tratti descritti altrimenti
in forma libera, un livello lemmatico permetterebbe di interrogare il
corpus per vocabolo lessicale a prescindere dalla particolare forma
flessa, restaurata o abbreviata in cui esso compare in una singola
epigrafe, aprendo la strada ad analisi lessicali sistematiche --- di
frequenza, di distribuzione geografica o cronologica di un termine, di
compresenza con altri lemmi --- che allo stato attuale la sola ricerca
per stringa non consente di condurre in maniera affidabile.
